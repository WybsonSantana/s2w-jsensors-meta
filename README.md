...
# Design Doc: Adoção da Arquitetura de Microsserviços no JSensors

## 1. Visão Geral

Este documento de design detalha a adoção de uma arquitetura de microsserviços para a aplicação JSensors, monitoramento de sensores, composta por três serviços:
...
Foram identificadas e mapeadas duas principais capacidades de negócio para a aplicação JSensors:
...
Com base na entrevista com Carlos, CEO da JSensors, identificamos os principais problemas enfrentados pela empresa e como a arquitetura de microsserviços proposta resolve cada um deles.
...
A arquitetura de microsserviços é desenhada e implementada para resolver esses problemas de forma eficiente e escalável. Abaixo, detalhamos como cada problema é abordado:
...
- **Solução**: A arquitetura de microsserviços permite a escalabilidade independente de cada serviço. O Temperature Processing Service é escalado horizontalmente para lidar com um grande volume de dados de sensores, enquanto o Temperature Monitoring Service é otimizado para consultas rápidas e geração de relatórios.
...
- **Benefício**: A empresa consegue detectar e responder rapidamente a variações críticas de temperatura, reduzindo riscos operacionais.

#### 5.2.4 Histórico de Temperaturas

- **Solução**: O Temperature Monitoring Service armazena todos os dados de temperatura recebidos do Temperature Processing Service em um banco de dados temporal (Postgres). Isso permite a consulta de históricos de temperatura por longos períodos.
- **Benefício**: A empresa consegue analisar tendências, gerar relatórios históricos e tomar decisões baseadas em dados.
...
### 5.4 Desenvolvimento dos Microsserviços

1. **Desenvolvimento dos Microsserviços**
   - O desenvolvimento prioriza o Device Management Service, unificando o gerenciamento dos sensores.
   - Em seguida, é desenvolvido o Temperature Processing Service, que recebe e processa dados em tempo real.
   - Por fim, é implementado o Temperature Monitoring Service, que realiza a análise de dados, geração de alertas e armazenamento de histórico.
2. **Integração com Sensores Existentes**
   - São desenvolvidos adaptadores para integrar os diferentes tipos de sensores ao Device Management Service e ao Temperature Processing Service.
3. **Implementação de Alertas**
   - O Temperature Monitoring Service é configurado para enviar alertas via e-mail, SMS ou integração com aplicativos de mensagens.

## 6. Conclusão

A arquitetura de microsserviços implementada resolve os principais problemas identificados na entrevista com o CEO da JSensors, oferecendo uma solução escalável, resiliente e de fácil manutenção. Com a centralização de dados, alertas em tempo real, histórico de temperaturas e configuração unificada de sensores, a empresa está mais bem preparada para atender às necessidades dos clientes e crescer de forma sustentável.
